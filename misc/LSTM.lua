...
function LSTM.lstm_bottomattention(lencoder_seq_length, batch_size, input_size, output_size, rnn_size, n, dropout)
  dropout = dropout or 0
  ...
  for L = 1,n do
    ...
    local decoder_attn = create_decoder_attn(rnn_size, 0, batch_size, lencoder_seq_length, 1)
    local decoder_out = decoder_attn({prev_h, inputs[#inputs]})
    x = nn.CAddTable()({x, decoder_out})
...
function LSTM.lstm_attend_last_hidden_state(input_size, output_size, rnn_size, n, dropout)
  dropout = dropout or 0
  ...
  for L = 1,n do
    ...
    x = nn.CAddTable()({x, nn.Linear(rnn_size, rnn_size)(inputs[#inputs])})
...
function LSTM.lstm_topattention(lencoder_seq_length, batch_size, input_size, output_size, rnn_size, n, dropout)
  dropout = dropout or 0
  ...
  local decoder_attn = create_decoder_attn(rnn_size, 0, batch_size, lencoder_seq_length, 0)
...
function LSTM.lstm_topattention_with_spatial(lencoder_seq_length, batch_size, input_size, output_size, rnn_size, n, dropout)
  dropout = dropout or 0
  ...
  local x, input_size_L
  local spatial_info = nn.Linear(4, rnn_size)(inputs[#inputs-1])
  local outputs = {}
  assert(n == 1)
  for L = 1,n do
    -- c,h from previos timesteps
