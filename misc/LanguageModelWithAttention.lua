...
local layer, parent = torch.class('nn.LanguageModelWithAttention', 'nn.Module')
function layer:__init(opt)
  ...
  self.core = LSTM.lstm_topattention(self.lencoder_seq_length, self.batch_size, self.input_encoding_size, self.vocab_size + 1, self.rnn_size, self.num_layers, dropout)
...
function layer:sample(context, opt)
  ...
  if sample_max == 1 and beam_size > 1 then return self:sample_beam(context, opt) end -- indirection for beam search
...
function layer:updateGradInput(input, gradOutput)
  ...
  self.gradInput = {dcontext, torch.Tensor()}
...
-- local crit, parent = torch.class('nn.LanguageModelWithAttentionCriterion', 'nn.Criterion')
