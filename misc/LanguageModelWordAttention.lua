...
local layer, parent = torch.class('nn.LanguageModelWordAttention', 'nn.Module')
function layer:__init(opt)
  ...
  print("langauge model batch_size: " .. self.batch_size)
  self.lencoder_seq_length = utils.getopt(opt, 'lencoder_seq_length', 1)
  print("lencoder_seq_length: " .. self.lencoder_seq_length)
  local dropout = utils.getopt(opt, 'dropout', 0)
  -- options for Language Model
  self.seq_length = utils.getopt(opt, 'seq_length')
  -- create the core lstm network. note +1 for both the START and END tokens
  self.core = LSTM.lstm_bottomattention(self.lencoder_seq_length, self.batch_size, self.input_encoding_size, self.vocab_size + 1, self.rnn_size, self.num_layers, dropout)
...
function layer:createClones()
  ...
  -- lookup tables for attention
  self.lookup_tables_att = {}
  for t = 2,self.seq_length+2 do
     self.lookup_tables_att[t] = {}
     for i = 1,self.lencoder_seq_length do
        self.lookup_tables_att[t][i] = self.lookup_table:clone('weight', 'gradWeight')
     end
  end
end
...
--]]
function layer:sample(imgs, keyword_seq, opt)
  ...
  keyword_seq[torch.eq(keyword_seq,0)] = 1
  local keyword_seq_length = keyword_seq:size(1)
  local nsamples = keyword_seq:size(2)
  local x_attention = torch.FloatTensor(nsamples, keyword_seq_length, self.input_encoding_size):zero():cuda()
  for t=1,self.seq_length+2 do
    ...
    if t >=2 then
       local xx_attention = torch.FloatTensor(keyword_seq_length, nsamples, self.input_encoding_size):cuda()
       -- lookup tables for current time t, a list of lookup tables
       -- for each keyword
       local lookup_table_attention_t = self.lookup_tables_att[t]
       for keyword_idx = 1, keyword_seq:size(1) do
          -- TODO record looup_table inputs
          local it_attention = keyword_seq[keyword_idx]:clone() -- 10
          local xt_attention = lookup_table_attention_t[keyword_idx]:forward(it_attention) -- 10*768
          -- for m = 1, nsamples do
          --    if it_attention[m] == 0 then xt_attention[m]:zero() end
          -- end
          xx_attention[keyword_idx] = xt_attention
       end
       x_attention = nn.Transpose({1,2}):cuda():forward(xx_attention)
    end
    ...
    table.insert(inputs, x_attention)
...
function layer:updateOutput(input)
  local imgs = input[1]
  local seq = input[2]
  self.keyword_seq = input[3]
  self.keyword_seq[torch.eq(self.keyword_seq,0)] = 1
  -- TODO
  ...
  local keyword_seq_length = self.keyword_seq:size(1)
  local nsamples = self.keyword_seq:size(2)
  local x_attention = torch.FloatTensor(nsamples, keyword_seq_length, self.input_encoding_size):zero():cuda()
  for t=1,self.seq_length+2 do
    ...
    if t >= 2 and not can_skip then
       -- keyword_seq: number_keywords * number_samples
       if not can_skip then
          local xx_attention = torch.FloatTensor(keyword_seq_length, nsamples, self.input_encoding_size):cuda()
          -- lookup tables for current time t, a list of lookup tables
          -- for each keyword
          local lookup_table_attention_t = self.lookup_tables_att[t]
          for keyword_idx = 1, self.keyword_seq:size(1) do
             -- TODO record looup_table inputs
             -- TODO LookupTable's indices start from 1
             local it_attention = self.keyword_seq[keyword_idx]:clone() -- 10
             local xt_attention = lookup_table_attention_t[keyword_idx]:forward(it_attention) -- 10*768
             -- for m = 1, nsamples do
             --    if it_attention[m] == 0 then xt_attention[m]:zero() end
             -- end
    ...
    if not can_skip then
      ...
      table.insert(self.inputs[t], x_attention) -- x_attention: batch_size x keyword_seq_length x word_embedding_size
      -- forward the network
...
function layer:updateGradInput(input, gradOutput)
  ...
  for t=self.tmax,1,-1 do
    ...
    local dx_attention = dinputs[4] -- batch_size x keyword_seq_length x word_embedding_size
    ...
    if t >= 2 then
       local lookup_table_attention_t = self.lookup_tables_att[t]
       local dxx_attention = nn.Transpose({1,2}):cuda():forward(dx_attention) -- keyword_seq_length x batch_size x word_embedding_size
       for keyword_idx = 1, self.keyword_seq:size(1) do
          self.lookup_tables_att[t][keyword_idx]:backward(self.keyword_seq[keyword_idx], dxx_attention[keyword_idx])
       end
    end
  end
...
-- local crit, parent = torch.class('nn.LanguageModelWordAttentionCriterion', 'nn.Criterion')
